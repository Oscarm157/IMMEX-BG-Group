...
export function ProgressIndicator({ phase }: Props) {
  ...
  return (
    <div className="crm-card mt-10 overflow-hidden p-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <span className="relative flex h-2.5 w-2.5">
            <span className="absolute inline-flex h-full w-full animate-ping rounded-full opacity-70" style={{ background: "var(--crm-wine)" }} />
            <span className="relative inline-flex h-2.5 w-2.5 rounded-full" style={{ background: "var(--crm-wine)" }} />
          </span>
      ...
      <div className="relative mt-4 h-1.5 w-full overflow-hidden rounded-full bg-[var(--crm-surface-2)]">
        <motion.div animate={{ width: `${percent}%` }} transition={{ type: "spring", stiffness: 200, damping: 26 }} className="h-full rounded-full" style={{ background: "var(--crm-wine)" }} />
      </div>
