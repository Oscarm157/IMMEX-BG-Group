"use client";

import Link from "next/link";
import { motion } from "motion/react";
import { fmtDate } from "@/lib/crm-format";
import { OwnerChip, SourceBadge } from "./status";
import type { BoardLead } from "./BoardView";

export function BoardCard({
  lead,
  ownerName,
  draggable,
  dragging,
  onDragStart,
  onDragEnd,
}: {
  lead: BoardLead;
  ownerName: string | null;
  draggable: boolean;
  dragging: boolean;
  onDragStart: () => void;
  onDragEnd: () => void;
}) {
  const ql = lead.qualification ?? {};
  const interest = ql.service ?? ql.industry ?? null;
  const volume = ql.monthlyVolume ?? null;

  return (
    <motion.div
      layout
      initial={{ opacity: 0, y: 4 }}
      animate={{ opacity: dragging ? 0.4 : 1, y: 0 }}
      transition={{ duration: 0.18, ease: [0.22, 1, 0.36, 1] }}
      draggable={draggable}
      onDragStart={(e) => {
        // motion forwards the native DragEvent; set payload for the column drop.
        (e as unknown as DragEvent).dataTransfer?.setData("text/plain", lead.id);
        if ((e as unknown as DragEvent).dataTransfer) {
          (e as unknown as DragEvent).dataTransfer!.effectAllowed = "move";
        }
        onDragStart();
      }}
      onDragEnd={onDragEnd}
      className={`group relative rounded-xl border border-[var(--crm-line)] bg-[var(--crm-surface)] p-3 shadow-[0_1px_2px_rgba(20,18,14,0.04)] transition-shadow ${
        draggable ? "cursor-grab active:cursor-grabbing hover:border-[var(--crm-line-strong)] hover:shadow-[0_4px_14px_rgba(125,17,30,0.08)]" : ""
      }`}
    >
      <div className="flex items-start justify-between gap-2">
        <Link
          href={`/admin/${lead.id}`}
          draggable={false}
          className="text-[13.5px] font-semibold leading-tight text-[var(--crm-ink)] hover:text-[var(--crm-wine)] before:absolute before:inset-0 before:content-['']"
        >
          {lead.name ?? "No name"}
        </Link>
        <SourceBadge source={lead.source} />
      </div>

      {(lead.email || lead.phone) && (
        <div className="mt-1 flex flex-col gap-0.5 text-[11.5px] leading-tight text-[var(--crm-ink-soft)]">
          {lead.email && <span className="truncate">{lead.email}</span>}
          {lead.phone && <span>{lead.phone}</span>}
        </div>
      )}

      {(interest || volume) && (
        <div className="relative z-[1] mt-2 flex flex-wrap gap-1">
          {interest && (
            <span className="rounded-md bg-[var(--crm-surface-2)] px-1.5 py-0.5 text-[11px] text-[var(--crm-ink-soft)]">
              {interest}
            </span>
          )}
          {volume && (
            <span className="rounded-md border border-[var(--crm-wine-ring)] bg-[var(--crm-wine-tint)] px-1.5 py-0.5 text-[11px] text-[var(--crm-wine)]">
              {volume}
            </span>
          )}
        </div>
      )}

      <div className="relative z-[1] mt-2.5 flex items-center justify-between gap-2 border-t border-[var(--crm-line)] pt-2">
        {ownerName ? (
          <OwnerChip name={ownerName} id={lead.assignedTo ?? ownerName} />
        ) : (
          <span className="text-[11px] text-[var(--crm-ink-mute)]">Unassigned</span>
        )}
        <span className="text-[11px] text-[var(--crm-ink-mute)]">{fmtDate(lead.createdAt)}</span>
      </div>
    </motion.div>
  );
}
