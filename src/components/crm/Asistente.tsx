...
/**
 * Asistente de pauta: conversa y además mueve la pantalla (filtra, ordena, selecciona).
...
const SUGERENCIAS = [
  ...
  "Arma un grupo de despacho aduanal en Tijuana",
];
...
export function Asistente({ puedeGuardar }: { puedeGuardar: boolean }) {
  ...
  function arrastrar(e: React.MouseEvent) {
    ...
    const mover = (ev: MouseEvent) => {
      setAncho(Math.min(ANCHO_MAX, Math.max(ANCHO_MIN, window.innerWidth - ev.clientX)));
    };
    const soltar = () => {
  ...
  function aplicarAccion(nombre: string, input: Record<string, unknown>): string {
    switch (nombre) {
      case "aplicar_filtros": {
        ...
        // Los topes aceptan null para quitarse: undefined es "no lo toques".
        for (const [campo, clave, etiqueta] of [
          ["min_volumen", "minVolumen", "volumen desde"],
          ["max_volumen", "maxVolumen", "volumen hasta"],
          ["min_cpc", "minCpc", "puja desde"],
          ["max_cpc", "maxCpc", "puja hasta"],
        ] as const) {
          if (!(campo in input)) continue;
          const v = input[campo];
          if (v === null) {
            parcial[clave] = null;
            partes.push(`sin ${etiqueta}`);
          } else if (typeof v === "number") {
            parcial[clave] = v;
            partes.push(`${etiqueta} ${campo.endsWith("cpc") ? `$${v}` : num(v)}`);
          }
        }
        if (Array.isArray(input.competencias)) {
          parcial.competencias = input.competencias;
          partes.push(
            input.competencias.length
              ? `competencia ${input.competencias.join(", ")}`
              : "toda competencia",
          );
        }
      ...
      case "navegar": {
        ...
        if (input.servicio) p.set("servicio", String(input.servicio));
        ...
        for (const linea of lineas) {
          ...
          if (evento.type === "texto") {
            ...
            resultados.push({
              ...
              content: puedeGuardar
                ? "Propuesta mostrada. El usuario decide si la guarda."
                : "Propuesta mostrada. Este usuario no tiene permiso para guardar grupos.",
            });
  ...
  // Solo en Keywords y Grupos: en Leads o Blog no tiene datos que mover ni consultar,
    ...
    >
        ...
        {turnos.length === 0 && (
          <div className="space-y-2">
            <p className="text-[13px] leading-relaxed text-[var(--crm-ink-soft)]">
              Puedo mover la tabla por usted y ayudarle a decidir dónde pautar con los números que
              ya están medidos.
            </p>
            {SUGERENCIAS.map((s) => (
              <button
                    ...
                    >
                      ...
                      <div className="bg-[var(--crm-surface-2)] p-3">
                        ...
                        <p className="mt-0.5 text-[12px] text-[var(--crm-ink-mute)]">
                          {etiquetaServicio(c.input.servicio)} · {c.input.plaza ?? "Nacional"} ·{" "}
                          {c.input.keywords.length} keywords
                        </p>
                        {c.input.porque && (
                        ...
                        {c.resuelta ? (
                          ...
                          <p className="mt-2.5 text-[12.5px] text-[var(--crm-ink-mute)]">
                            Guardar grupos es de administrador. Pídale a un admin que lo cree.
                          </p>
                        )}
                      </div>
                    </div>
      ...
      >
        <textarea
          ...
          rows={2}
          placeholder="Pídame filtrar, seleccionar o dónde pautar"
  ...
  return (
    <div className="flex items-start gap-2.5 rounded-[var(--crm-r-md)] border border-[color-mix(in_srgb,var(--crm-accent)_35%,transparent)] bg-[color-mix(in_srgb,var(--crm-accent)_10%,transparent)] px-3 py-2.5">
      ...
      <span className="min-w-0 flex-1 text-[12.5px] leading-snug text-[var(--crm-ink)]">
        <span className="font-semibold text-[var(--crm-accent-strong)]">{a.label}.</span> {texto}
      </span>
    </div>
  );
}
