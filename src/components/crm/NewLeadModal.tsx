      ...
      <Modal open={open} onClose={() => setOpen(false)} title="Nuevo lead" maxWidth={400}>
        <form action={createLead} className="space-y-3.5">
          <div>
            ...
            <input id="nl-name" name="name" value={name} onChange={(e) => setName(e.target.value)} className="crm-input" placeholder="Full name" />
          </div>
          <div>
            <label className={labelCls} htmlFor="nl-email">Email</label>
            <input id="nl-email" name="email" type="email" value={email} onChange={(e) => setEmail(e.target.value)} className="crm-input" placeholder="name@company.com" />
          </div>
          <div>
