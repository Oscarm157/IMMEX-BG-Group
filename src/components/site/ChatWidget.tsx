          ...
          return (
            <Fragment key={step.key}>
              <div className="flex items-center gap-1.5">
                <span className={`size-1.5 rounded-full transition-colors ${filled ? 'bg-accent' : 'bg-line'}`} />
  ...
  if (role === 'user') {
    return (
      <div className="flex justify-end">
        <div className="max-w-[80%] rounded-2xl rounded-tr-sm bg-accent px-3.5 py-2.5 text-sm leading-relaxed text-on-accent">
          {content}
        </div>
      </div>
    );
      ...
      <AnimatePresence>
        {showProactiveBubble && !isOpen && (
          <motion.div
            ...
            className="fixed bottom-[152px] right-5 z-50 max-w-[230px] md:bottom-[92px] md:right-8"
          >
        ...
        {isOpen && (
          ...
          >
            ...
            <div className="flex shrink-0 items-center justify-between border-b border-line bg-surface-2 px-4 py-3">
              <div>
                <p className="font-mono text-[10px] font-medium uppercase tracking-[0.18em] text-ash">BG Consulting Group</p>
              ...
              {messages.length === 0 && (
                ...
                >
                  ...
                  <p className="px-2 text-[13px] leading-relaxed text-bone">
                    {locale === 'es'
                      ? 'Soy el asistente de BG Consulting Group. Puede preguntarme sobre clasificación, pedimentos, IMMEX, cumplimiento y qué servicio conviene a su caso.'
                      : 'I am the BG Consulting Group assistant. Ask about classification, customs entries, IMMEX, compliance and which service fits your case.'}
                  </p>
                  ...
                  >
                    {suggestedReplies.map((option) => (
                      <button
                        ...
                        className="rounded-full border border-accent/30 px-3 py-1.5 text-[12px] text-accent transition-colors hover:bg-accent/5"
                      >
                        {option}
                      </button>
                    ))}
            ...
            <div className="flex shrink-0 items-center gap-2 border-t border-line bg-surface-1 px-3 py-3">
              ...
              <motion.button
                ...
                whileTap={{ scale: 0.9 }}
                className="flex size-8 shrink-0 items-center justify-center rounded-full bg-accent text-on-accent transition-opacity disabled:cursor-not-allowed disabled:opacity-35"
                aria-label="Enviar"
              >
      ...
      <AnimatePresence>
        {!isOpen && (
          <motion.button
            ...
            whileTap={{ scale: 0.93 }}
            className="signal-glow fixed bottom-[88px] right-5 z-50 flex size-14 items-center justify-center rounded-full bg-accent text-on-accent md:bottom-6 md:right-8"
