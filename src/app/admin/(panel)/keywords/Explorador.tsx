...
export function Explorador({
  ...
  function ordenarPor(col: Columna) {
    setOrden(
      orden.col === col
        ? { col, desc: !orden.desc }
        : { col, desc: col !== "keyword" && col !== "servicio" },
    );
  }
      ...
      {seleccion.length > 0 && (
        <div className="crm-card mb-3 flex flex-wrap items-center gap-x-6 gap-y-2 p-3">
          ...
          <div className="ml-auto flex items-center gap-2">
            <AgregarAGrupo seleccion={seleccion} grupos={grupos} onListo={limpiarSeleccion} />
          ...
          <table className="crm-table min-w-[900px]">
            <thead className="crm-thead">
              <tr>
                ...
                <th className="crm-th text-right">SEO</th>
              </tr>
            </thead>
            <tbody key={gen}>
              {enPantalla.map((k, i) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="crm-td crm-num whitespace-nowrap text-right text-[13px] text-[var(--crm-ink-soft)]">
                      {k.cpc > 0
                        ? k.cpcBaja > 0
                    ...
                    {/* Dificultad e intención solo vienen de SEMrush; en las del Planner van vacías. */}
                    <td className="crm-td text-right text-[12.5px] text-[var(--crm-ink-mute)]">
                      {k.dificultad != null ? (
                        <span className="crm-num">
                          {k.dificultad}
                          {k.intencion && ` · ${INTENCION[k.intencion] ?? k.intencion}`}
                        </span>
                      ) : (
                        "—"
                      )}
