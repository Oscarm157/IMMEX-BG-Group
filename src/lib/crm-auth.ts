// Pure auth primitives for the admin panel (no Next imports), safe in Edge
// middleware and Node server actions. Password hashing via PBKDF2 (Web Crypto),
// session as a signed `uid.iat.HMAC` cookie.

export const CRM_COOKIE = "bg_session";
...
function secret(): string {
  ...
  if (!s || s.length < 16) {
    throw new Error("CRM_SECRET is not set (or too short); cannot sign sessions.");
  }
  return s;
}
